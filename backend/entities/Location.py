...
class Location:
    def __init__(self, id=None, qid=None, name=None, latitude=None, longitude=None, cursor=None):
    ...
    def _getFromTable(self):
        ...
        if not row and self.name and self.latitude is not None and self.longitude is not None:
            self.cursor.execute("""
                SELECT id, name, lat, lon, qid, type_id
                FROM locations
    ...
    def setData(self, data):
        ...
        if data["instance_label"]:
            locationtype_database_entity = LocationType(label=data["instance_label"], cursor=self.cursor)
            if locationtype_database_entity.id is None:
                try:
                    locationtype_database_entity.setData({"label": data["instance_label"]})
